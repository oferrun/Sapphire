...
public:
    ...
    bool IsKeyDown(InputKeys Key) const
    {
     ...
     bool IsKeyReleased(InputKeys Key) const
    {
        return (GetKeyState(Key) & INPUT_KEY_STATE_FLAG_KEY_WAS_DOWN) != 0;
    }

    bool IsKeyFirstDown(InputKeys Key) const
    {
        return (GetKeyState(Key) & INPUT_KEY_STATE_FLAG_KEY_FIRST_DOWN) != 0;
    }
    ...
    void ClearState()
    {
        ...
        for (Uint32 i = 0; i < static_cast<Uint32>(InputKeys::TotalKeys); ++i)
        {
            ...
            if (KeyState & INPUT_KEY_STATE_FLAG_KEY_FIRST_DOWN)
            {
                KeyState &= ~INPUT_KEY_STATE_FLAG_KEY_FIRST_DOWN;
            }
        }
    }
